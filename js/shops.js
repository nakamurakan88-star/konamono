// ============================================
// 店舗一覧ページ用 JavaScript v2
// ============================================

document.addEventListener('DOMContentLoaded', async () => {
  await checkAuth();
  const params = new URLSearchParams(window.location.search);
  if (params.get('pref')) document.getElementById('filter-pref').value = params.get('pref');
  if (params.get('style')) document.getElementById('filter-style').value = params.get('style');
  await loadShops();
});
...
async function loadShops() {
  ...
  countEl.textContent = `${shopsWithScore.length} 件の店舗が見つかりました`;
